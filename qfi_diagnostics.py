...
# ═══════════════════════════════════════════════════════════════
# 1. ДАТАСЕТ: демонстрационный размер
# ═══════════════════════════════════════════════════════════════
def make_dataset(n_samples=24, noise=0.15, seed=42):
    """
    Датасет не 4 произвольные точки, а make_moons (типичная для ML форма — два полумесяца с нелинейной границей, но демонстрационный
    размер 24 точки, чтобы QFI считался на CPU за секунды).
    """
...
def make_circuit(n_qubits: int, n_layers: int):
    ...
    Кодирование необучаемое (requires_grad=False), поэтому в QFI попадают только вариационные параметры theta.
    """
...
def extract_qfim(qfi_result):
    """
    PennyLane возвращает QFIM либо напрямую (один trainable аргумент), либо в виде tuple (по одной матрице на каждый trainable аргумент схемы).
    В этом скрипте x имеет requires_grad=False, поэтому trainable аргумент только один (theta) — и tuple обычно не возникает. Но если убрать
    requires_grad=False у датасета, quantum_fisher вернет tuple; тогда вместо неочевидного TypeError мы явно берем первый элемент — QFIM по theta.
    """
...
def run_experiment(name, n_qubits, n_layers, X_scaled, seed=42):
    ...
    # Приводим к чистому numpy для совместимости с linalg и визуализацией.
    # Осознанный отказ от autograd здесь безопасен: дальше QFIM только анализируется (собственные значения, диагональ), но не
    # дифференцируется повторно. Если понадобится расширить эксперимент на градиентные методы поверх самой QFIM, то эту строку нужно убрать.
    qfim = np.array(qfim)
...
# ═══════════════════════════════════════════════════════════════
# 5. СРАВНЕНИЕ ГЛУБИНЫ: фиксированное число кубитов
# ═══════════════════════════════════════════════════════════════
def run_depth_scan(n_qubits, layer_list, X_scaled, seed=42):
    """
    В отличие от сравнения Toy/Standard/Deep (где меняются И число кубитов, И число слоев одновременно), здесь кубиты зафиксированы —
    это изолирует именно эффект глубины, а не архитектуры в целом.
    Отвечает на вопрос "сколько слоев реально нужно?" без примеси эффекта от увеличения ширины схемы.
    """
    results = []
...
def print_depth_scan_table(depth_results, n_qubits):
    ...
    print("="*75)
    print(f"| Слоев | Параметров | Средняя чувствительность | Прирост |")
    print(f"|---|---|---|---|")
    prev = None
...
def run_svm_correlation(X_scaled, y, n_qubits, n_layers, seed=42):
    ...
    2. Для каждой точки считаем "индекс напряженности" — след QFI,
       деленный на число параметров (та же нормировка, что и в compute_ml_metrics).
    3. Считаем корреляцию Пирсона (линейная связь) И корреляцию Спирмена (ранговая, устойчива к выбросам и нелинейным, но
       монотонным связям) между |decision_function| SVM (расстояние до границы) и QFI-чувствительностью точки — вторая метрика
       нужна, чтобы результат не держался на одном выбросе или случайной линейной аппроксимации немонотонной связи.

    Умеренная корреляция — ожидаемый и содержательный результат, т.к. QFI измеряет чувствительность именно квантовой схемы.
    """
    ...
    spearman_corr, _ = spearmanr(svm_distance, per_point_sens)
...
def plot_article_figure(results, save_path="figures/qfi_ml_diagnostics.png"):
    ...
    # Эвристический индекс модульности, а не строго нормированная метрика:
    # cross_layer_corr — среднее АБСОЛЮТНЫХ значений, не гарантированно
    # ограничено [0,1]. В наблюдаемых прогонах (0.03–0.12) это безопасно,
    # но при других архитектурах/датасетах "modularity" в принципе может
    # выйти за пределы [0,1] — тогда бар нужно клипать или перенормировать.
...
def plot_reconnaissance_figure(X_raw, y, svm_result, save_path="figures/qfi_reconnaissance.png"):
    ...
    # Граница решений классического SVM (для контекста, не как истина в последней инстанции для QFI — это разные вещи, см. текст статьи)
    x_min, x_max = X_raw[:, 0].min() - 0.3, X_raw[:, 0].max() + 0.3
    y_min, y_max = X_raw[:, 1].min() - 0.3, X_raw[:, 1].max() + 0.3
    xx, yy = np.meshgrid(np.linspace(x_min, x_max, 200), np.linspace(y_min, y_max, 200))

    # SVM был обучен на масштабированных в [0, π] данных — пересчитываем сетку туда же
    ...
    ax.set_title(f"Квантовая разведка датасета\n"
                 f"Pearson r = {svm_result['correlation']:.2f}, "
                 f"Spearman r = {svm_result['spearman']:.2f}")
...
if __name__ == "__main__":
    ...
    # Датасет демонстрационного размера (см. раздел 1)
    ...
    print(f"Корреляция Спирмена (та же пара, ранговая, устойчива к выбросам): "
          f"{svm_result['spearman']:.3f}")
