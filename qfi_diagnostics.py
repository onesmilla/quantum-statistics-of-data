...
def run_experiment(name, n_qubits, n_layers, X_scaled, seed=42):
    ...
    # Осознанный отказ от autograd здесь безопасен: дальше QFIM только
    # анализируется (собственные значения, диагональ), но не
    # дифференцируется повторно. Если понадобится расширить эксперимент
    # на градиентные методы поверх самой QFIM — эту строку нужно убрать.
    qfim = np.array(qfim)
...
def run_svm_correlation(X_scaled, y, n_qubits, n_layers, seed=42):
    ...
    2. Для каждой точки считаем "индекс напряжённости" — след QFI,
       ...
       compute_ml_metrics).
    3. Считаем корреляцию Пирсона (линейная связь) И корреляцию
       Спирмена (ранговая, устойчива к выбросам и нелинейным, но
       монотонным связям) между |decision_function| SVM (расстояние
       до границы) и QFI-чувствительностью точки — вторая метрика
       нужна, чтобы результат не держался на одном выбросе или
       случайной линейной аппроксимации немонотонной связи.
